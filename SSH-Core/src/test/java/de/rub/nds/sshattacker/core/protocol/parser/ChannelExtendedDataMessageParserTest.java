...
public class ChannelExtendedDataMessageParserTest {
    ...
    public static Stream<Arguments> provideTestVectors() {
        return Stream.of(Arguments.of(ArrayConverter.hexStringToByteArray("5F000000000000000100000004DEADBEEF"), 0,
                ExtendedChannelDataType.SSH_EXTENDED_DATA_STDERR, ArrayConverter.hexStringToByteArray("DEADBEEF")),
                Arguments.of(ArrayConverter.hexStringToByteArray("5F000000010000000100000008DEADBEEFDEADBEEF"), 1,
                        ExtendedChannelDataType.SSH_EXTENDED_DATA_STDERR,
                        ArrayConverter.hexStringToByteArray("DEADBEEFDEADBEEF")), Arguments.of(
                        ArrayConverter.hexStringToByteArray("5FFFFFFFFF0000000100000004DEADC0DE"),
                        Integer.parseUnsignedInt("FFFFFFFF", 16), ExtendedChannelDataType.SSH_EXTENDED_DATA_STDERR,
    ...
    public void testParse(byte[] providedBytes, int expectedRecipientChannel, ExtendedChannelDataType expectedDataType,
            byte[] expectedPayload) {
        ChannelExtendedDataMessageParser parser = new ChannelExtendedDataMessageParser(0, providedBytes);
        ChannelExtendedDataMessage msg = parser.parse();

        assertEquals(MessageIDConstant.SSH_MSG_CHANNEL_EXTENDED_DATA.id, msg.getMessageID().getValue());
