...
public class ChannelSuccessMessageParserTest {
    ...
    public static Stream<Arguments> provideTestVectors() {
        return Stream.of(Arguments.of(ArrayConverter.hexStringToByteArray("6300000000"), 0),
    ...
    public void testParse(byte[] providedBytes, int expectedRecipientChannel) {
        ChannelSuccessMessageParser parser = new ChannelSuccessMessageParser(0, providedBytes);
        ChannelSuccessMessage msg = parser.parse();

        assertEquals(MessageIDConstant.SSH_MSG_CHANNEL_SUCCESS.id, msg.getMessageID().getValue());
