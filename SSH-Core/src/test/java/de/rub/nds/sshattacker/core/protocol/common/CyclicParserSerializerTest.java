...
public class CyclicParserSerializerTest {
    ...
    // ToDo testParserSerializerPairs is equal to the Test with DefaultConstructors at the moment.
    // If the classes are constructed from the config file
        ...
        for (Class<? extends ProtocolMessageParser> someParserClass : parserClasses) {
            ...
            try {
                ...
                try {
                    ...
                    preparator =
                            (ProtocolMessagePreparator)
                                    getConstructor(preparatorClass, 2)
                ...
                Class<? extends ProtocolMessageSerializer> serializerClass =
                        getSerializer(testName);
                try {
                    serializer =
                            (ProtocolMessageSerializer)
                                    getConstructor(serializerClass, 1).newInstance(message);
                } catch (SecurityException
                        | InstantiationException
                        | IllegalAccessException
                        | IllegalArgumentException
                ...
                try {
                    parser =
                            (ProtocolMessageParser)
                                    getConstructor(someParserClass, 2)
                                            .newInstance(serializedMessage, 0);
                } catch (SecurityException
                        | InstantiationException
                        | IllegalAccessException
                        | IllegalArgumentException
                ...
                Assert.assertArrayEquals(
                        testName + " failed", serializedMessage, serializer.serialize());
            ...
            try {
                ...
                try {
                    Constructor tempConstructor =
                            getDefaultMessageConstructor(protocolMessageClass);
                    if (tempConstructor != null) {
                        message =
                                (ProtocolMessage)
                                        getDefaultMessageConstructor(protocolMessageClass)
    ...
    private Class<? extends ProtocolMessage> getProtocolMessage(String testName)
            throws ClassNotFoundException {
        String[] messageNames = {
            "de.rub.nds.sshattacker.core.protocol.authentication.message." + testName,
            "de.rub.nds.sshattacker.core.protocol.connection.message." + testName,
            "de.rub.nds.sshattacker.core.protocol.transport.message." + testName
        };
        for (String messageName : messageNames) {
            try {
                ...
                try {
                    return (Class<? extends ProtocolMessage>)
                            Class.forName(messageName + "Message");
                } catch (ClassNotFoundException ex) {
                }
            }
        }
    ...
    private Class<? extends ProtocolMessagePreparator> getPreparator(String testName)
            throws ClassNotFoundException {
        String[] preparatorNames = {
            "de.rub.nds.sshattacker.core.protocol.authentication.preparator." + testName,
            "de.rub.nds.sshattacker.core.protocol.connection.preparator." + testName,
            "de.rub.nds.sshattacker.core.protocol.transport.preparator." + testName
        };
        for (String preparatorName : preparatorNames) {
            try {
                return (Class<? extends ProtocolMessagePreparator>)
                        Class.forName(preparatorName + "Preparator");
            } catch (ClassNotFoundException E) {
                try {
                    return (Class<? extends ProtocolMessagePreparator>)
                            Class.forName(preparatorName + "MessagePreparator");
                } catch (ClassNotFoundException ex) {
                }
            }
        }
    ...
    private Class<? extends ProtocolMessageSerializer> getSerializer(String testName)
            throws ClassNotFoundException {
        String[] serializerNames = {
            "de.rub.nds.sshattacker.core.protocol.authentication.serializer." + testName,
            "de.rub.nds.sshattacker.core.protocol.connection.serializer." + testName,
            "de.rub.nds.sshattacker.core.protocol.transport.serializer." + testName
        };
        for (String serializerName : serializerNames) {
            try {
                return (Class<? extends ProtocolMessageSerializer>)
                        Class.forName(serializerName + "Serializer");
            } catch (ClassNotFoundException E) {
                try {
                    return (Class<? extends ProtocolMessageSerializer>)
