...
public class ServerSigAlgsExtensionParser extends AbstractExtensionParser<ServerSigAlgsExtension> {
    ...
    private void parseAcceptedPublicKeyAlgorithms() {
        ModifiableInteger length = extension.getValueLength();
        LOGGER.debug("Accepted public key algorithms length: {}", length.getValue());

        extension.setAcceptedPublicKeyAlgorithms(
                super.parseByteString(length.getValue(), StandardCharsets.US_ASCII));
        LOGGER.debug(
