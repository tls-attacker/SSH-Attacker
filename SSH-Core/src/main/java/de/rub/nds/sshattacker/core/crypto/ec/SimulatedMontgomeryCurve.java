...
public class SimulatedMontgomeryCurve extends EllipticCurveOverFp {
    ...
    private EllipticCurveOverFp computeWeierstrassEquivalent() {
        BigInteger weierstrassA = new BigInteger("3").subtract(this.getFieldA().getData()
        ...
        BigInteger weierstrassB = new BigInteger("2").multiply(
                this.getFieldA().getData().modPow(new BigInteger("3"), this.getModulus())).subtract(
                new BigInteger("9").multiply(this.getFieldA().getData()));
        weierstrassB = weierstrassB.multiply(
                new BigInteger("27")
                        .multiply(this.getFieldB().getData().modPow(new BigInteger("3"), this.getModulus()))
                        .modInverse(this.getModulus())).mod(this.getModulus());

        Point weierstrassGen = toWeierstrass(this.getBasePoint());
        return new EllipticCurveOverFp(weierstrassA, weierstrassB, this.getModulus(), weierstrassGen.getFieldX()
                .getData(), weierstrassGen.getFieldY().getData(), this.getBasePointOrder());
    }
    ...
    public Point toWeierstrass(Point mpoint) {
        if (mpoint.isAtInfinity()) {
            ...
            BigInteger weierstrassX = mx
                    .multiply(this.getFieldB().getData().modInverse(this.getModulus()))
                    .add(this
                            .getFieldA()
                            .getData()
                            .multiply(
                                    new BigInteger("3").multiply(this.getFieldB().getData()).modInverse(
                                            this.getModulus()))).mod(this.getModulus());
            BigInteger weierstrassY = my.multiply(this.getFieldB().getData().modInverse(this.getModulus())).mod(
                    this.getModulus());
            ...
            BigInteger mx = weierstrassX
                    .subtract(
                            this.getFieldA()
                                    ...
                                    .multiply(
                                            new BigInteger("3").multiply(this.getFieldB().getData()).modInverse(
                                                    this.getModulus()))).multiply(this.getFieldB().getData())
                    .mod(this.getModulus());
