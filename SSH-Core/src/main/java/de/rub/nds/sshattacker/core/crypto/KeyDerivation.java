...
public class KeyDerivation {
    ...
    public static void deriveKeys(SshContext context) {
        ...
        context.setInitialIvClientToServer(KeyDerivation.deriveKey(keyExchange.getSharedSecret(),
                exchangeHash, 'A', sessionId, context
        ...
        context.setInitialIvServerToClient(KeyDerivation.deriveKey(keyExchange.getSharedSecret(),
                exchangeHash, 'B', sessionId, context
        ...
        context.setEncryptionKeyClientToServer(KeyDerivation.deriveKey(keyExchange.getSharedSecret(), exchangeHash, 'C', sessionId, context.getCipherAlgorithmClientToServer()
        ...
        context.setEncryptionKeyServerToClient(KeyDerivation.deriveKey(keyExchange.getSharedSecret(), exchangeHash, 'D', sessionId, context.getCipherAlgorithmServerToClient()
                .orElseThrow(AdjustmentException::new).getKeySize(), hashAlgorithm));
        LOGGER.debug("Key D: " + ArrayConverter.bytesToRawHexString(context.getEncryptionKeyServerToClient().orElse(new byte[0])));
        context.setIntegrityKeyClientToServer(KeyDerivation.deriveKey(keyExchange.getSharedSecret(), exchangeHash, 'E', sessionId, context.getMacAlgorithmClientToServer()
                .orElseThrow(AdjustmentException::new).getKeySize(), hashAlgorithm));
        LOGGER.debug("Key E: " + ArrayConverter.bytesToRawHexString(context.getIntegrityKeyClientToServer().orElse(new byte[0])));
        context.setIntegrityKeyServerToClient(KeyDerivation.deriveKey(keyExchange.getSharedSecret(), exchangeHash, 'F', sessionId, context.getMacAlgorithmServerToClient()
                .orElseThrow(AdjustmentException::new).getKeySize(), hashAlgorithm));
        LOGGER.debug("Key F: " + ArrayConverter.bytesToRawHexString(context.getIntegrityKeyServerToClient().orElse(new byte[0])));
    }

    static byte[] deriveKey(BigInteger sharedSecret, byte[] exchangeHash, char label, byte[] sessionID, int outputLen,
            String hashFunction) {
        ...
        try {
            ...
            outStream.write(md.digest(Arrays.concatenate(serializedSharedSecret, exchangeHash,
                    new byte[] { (byte) label }, sessionID)));
