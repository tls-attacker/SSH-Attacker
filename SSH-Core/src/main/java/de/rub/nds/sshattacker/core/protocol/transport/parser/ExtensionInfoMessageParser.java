...
public class ExtensionInfoMessageParser extends SshMessageParser<ExtensionInfoMessage> {
    ...
    private void parseExtensions() {
        for (int extensionIndex = 0, extensionStartPointer = getPointer();
            ...
            AbstractExtensionParser<?> extensionParser = switch (extension) {
                case SERVER_SIG_ALGS -> new ServerSigAlgsExtensionParser(getArray(), extensionStartPointer);
                case DELAY_COMPRESSION -> new DelayCompressionExtensionParser(getArray(), extensionStartPointer);
                case PING_OPENSSH_COM -> new PingExtensionParser(getArray(), extensionStartPointer);
                case PUBLICKEY_ALGORITHMS_ROUMENPETROV -> new PublicKeyAlgorithmsRoumenPetrovExtensionParser(
