...
public class Sntrup761X25519KeyExchangeInitMessageSerializer
    ...
    public Sntrup761X25519KeyExchangeInitMessageSerializer(Sntrup761X25519KeyExchangeInitMessage message) {
        super(message);
    }

    @Override
    public void serializeMessageSpecificContents() {
        int length = message.getEphemeralECPublicKeyLength().getValue()
                + message.getEphemeralSNTRUPPublicKeyLength().getValue();
        byte[] keys = ArrayConverter.concatenate(message.getEphemeralSNTRUPPublicKey().getValue(),
        ...
        LOGGER.debug("x25519 || sntrup761 public Key (client): " + ArrayConverter.bytesToHexString(keys));
        appendBytes(keys);
    }
}
