...
public class PublicKeyAlgorithmsRoumenPetrovExtensionParser extends AbstractExtensionParser<PublicKeyAlgorithmsRoumenPetrovExtension> {
    ...
    private void parsePublicKeyAlgorithmsLength() {
        extension.setPublicKeyAlgorithmsLength(parseIntField(DataFormatConstants.STRING_SIZE_LENGTH));
        LOGGER.debug("Public key algorithms length: {}", extension.getPublicKeyAlgorithmsLength().getValue());
    }

    private void parsePublicKeyAlgorithms() {
        extension.setPublicKeyAlgorithms(parseByteString(extension.getPublicKeyAlgorithmsLength().getValue(), StandardCharsets.US_ASCII));
