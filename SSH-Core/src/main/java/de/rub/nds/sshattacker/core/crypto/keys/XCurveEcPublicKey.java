...
public class XCurveEcPublicKey implements PublicKey {
    ...
    public XCurveEcPublicKey(byte[] coordinate, NamedGroup group) {
        if (!group.isRFC7748Curve()) {
            throw new IllegalArgumentException("XCurveEcPublicKey does not support named group " + group);
        }
        if ((group == NamedGroup.ECDH_X25519 && coordinate.length != CryptoConstants.X25519_POINT_SIZE)
                || group == NamedGroup.ECDH_X448 && coordinate.length != CryptoConstants.X448_POINT_SIZE) {
