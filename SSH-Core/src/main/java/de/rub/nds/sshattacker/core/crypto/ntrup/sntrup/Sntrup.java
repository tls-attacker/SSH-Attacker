...
public class Sntrup {
    ...
    public SntrupKeyPair keyGen() {
        ...
        byte[] privKbytes;
        if (round1) {
            privKbytes = ArrayConverter.concatenate(encF, encV, encH);
        } else {
            privKbytes = ArrayConverter.concatenate(encF, encV, encH, roh, hashPrefixedB(encH, (byte) 4));
        }
    ...
    public void encaps(byte[] pubK) {
        ...
        byte[] hashConfirm;
        if (round1) {
            hashConfirm = sha512(encR);
            this.cyphertext = ArrayConverter.concatenate(hashConfirm, c);
            this.sharedSecret = hashConfirm;

        } else {
            byte[] cache = hashPrefixedB(pubK, (byte) 4);
            byte[] hashencR = hashPrefixedB(encR, (byte) 3);
            hashConfirm = hashPrefixedB(ArrayConverter.concatenate(hashencR, cache), (byte) 2);
    ...
    public byte[] decaps(byte[] privK, byte[] cyphertext) {
        ...
        Rounded c;
        if (round1) {
            c = Rounded.decode(set, Arrays.copyOfRange(cyphertext, 32, cyphertextBytes));
        } else {
            c = Rounded.decode(set, Arrays.copyOfRange(cyphertext, 0, cyphertextBytes));
        }
        ...
        if (Arrays.equals(cyphertext, cyphertextNew)) {
            ...
            LOGGER.warn(
                    "Could not decapsulate the shared secret, try to return return rho instead");
