...
public class X509EcdsaPublicKeyParser extends Parser<SshPublicKey<CustomX509EcdsaPublicKey, CustomEcPrivateKey>> {
    ...
    public SshPublicKey<CustomX509EcdsaPublicKey, CustomEcPrivateKey> parse() {
        try {
            ...
            if (publicKey instanceof ECPublicKey) {
                ...
                CustomX509EcdsaPublicKey customX509EcdsaPublicKey = new CustomX509EcdsaPublicKey(ecPublicKey.getW().getAffineX(),
                        ecPublicKey.getW().getAffineY(), signature, curveName, group);
                ...
                LOGGER.debug("Parsed ECDSA Public Key Point: x={}, y={}", ecPublicKey.getW().getAffineX(), ecPublicKey.getW().getAffineY());
                ...
                return new SshPublicKey<>(PublicKeyFormat.X509V3_ECDSA_SHA2_NISTP256, customX509EcdsaPublicKey);
            } else {
                throw new IllegalArgumentException("Invalid X.509 certificate - ECDSA key missing.");
            }
        } catch (Exception e) {
    ...
    // Utility methods for finding the X.509 start index, extracting the certificate, and parsing extensions
    ...
    private Map<String, String> parseExtensions(X509Certificate cert) {
        ...
        try {
            byte[] subjectKeyIdentifier = cert.getExtensionValue(Extension.subjectKeyIdentifier.getId());
            if (subjectKeyIdentifier != null) {
                extensionsMap.put("SubjectKeyIdentifier", bytesToHex(subjectKeyIdentifier));
            }

            byte[] authorityKeyIdentifier = cert.getExtensionValue(Extension.authorityKeyIdentifier.getId());
