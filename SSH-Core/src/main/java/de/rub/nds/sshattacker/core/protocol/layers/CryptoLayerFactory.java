...
public class CryptoLayerFactory {
    ...
    private static String getCipherTransformByAlgorithm(EncryptionAlgorithm encryptionAlgorithm) {
        switch (encryptionAlgorithm) {
            ...
            default:
                LOGGER.fatal(
                        "The following cipher algorithm was negotiated, but is not yet implemented: "
                                + encryptionAlgorithm);
                throw new NotImplementedException(
                        "CryptoLayerFactory::getCipherTransformByAlgorithm");
        }
    }
    ...
    private static String getMacTransformByAlgorithm(MacAlgorithm macAlgorithm) {
        switch (macAlgorithm) {
            ...
            default:
                LOGGER.fatal(
                        "The following mac algorithm was negotiated, but is not yet implemented: "
                                + macAlgorithm);
                throw new NotImplementedException("CryptoLayerFactory::getMacTransformByAlgorithm");
        }
    }

    private static AlgorithmParameterSpec getCipherParametersByAlgorithm(
            byte[] parameters, EncryptionAlgorithm encryptionAlgorithm) {
        switch (encryptionAlgorithm) {
            case AES128_CBC:
            case AES128_CTR:
            case AES192_CBC:
            ...
            case AES256_GCM_OPENSSH_COM:
                return new GCMParameterSpec(
                        MacAlgorithm.AEAD_AES_128_GCM.getOutputSize(), parameters);
            default:
                return null;
        }
    }
    ...
    public static CryptoLayer getCryptoLayer(boolean clientToServer, SshContext sshContext) {
        EncryptionAlgorithm encryptionAlgorithm =
                (clientToServer
                                ? sshContext.getCipherAlgorithmClientToServer()
        ...
        Key cipherKey =
                new SecretKeySpec(
                        (clientToServer
                                        ? sshContext.getEncryptionKeyClientToServer()
                                        : sshContext.getEncryptionKeyServerToClient())
                                .orElseThrow(IllegalArgumentException::new),
                        cipherTransform);
        Key macKey =
                new SecretKeySpec(
                        (clientToServer
                                        ? sshContext.getIntegrityKeyClientToServer()
                                        : sshContext.getIntegrityKeyServerToClient())
                                .orElseThrow(IllegalArgumentException::new),
                        macTransform);
        AlgorithmParameterSpec cipherParams =
                getCipherParametersByAlgorithm(
                        (clientToServer
                                        ? sshContext.getInitialIvClientToServer()
                                        : sshContext.getInitialIvServerToClient())
                                .orElseThrow(IllegalArgumentException::new),
                        encryptionAlgorithm);
