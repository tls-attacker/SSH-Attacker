...
import de.rub.nds.tlsattacker.core.state.SshContext;
...
public abstract class ProtocolMessageHandler<Message extends ProtocolMessage> extends Handler<Message> {
    ...
    public byte[] prepareMessage(Message message, boolean withPrepare) {
        ...
        try {
            if (message.getAdjustContext()) {
                ...
                if (sshContext.getConfig().getDefaultSelectedProtocolVersion().isDTLS() && message.isHandshakeMessage()
                        && !message.isDtlsHandshakeMessageFragment()) {
                    sshContext.setDtlsCurrentSendSequenceNumber(sshContext.getDtlsNextSendSequenceNumber());
                    sshContext.increaseDtlsNextSendSequenceNumber();
                }
            }
    ...
    private void updateDigest(ProtocolMessage message) {
        if (message.isHandshakeMessage() && ((HandshakeMessage) message).getIncludeInDigest()) {
            if (sshContext.getChooser().getSelectedProtocolVersion().isDTLS()) {
                DtlsHandshakeMessageFragment fragment = new MessageFragmenter(sshContext.getConfig())
                        .wrapInSingleFragment((HandshakeMessage) message, sshContext);
                LOGGER.debug("Included in digest fragmented version of: " + message.toCompactString());
                sshContext.getDigest().append(fragment.getCompleteResultingMessage().getValue());
            } else {
                LOGGER.debug("Included in digest: " + message.toCompactString());
                sshContext.getDigest().append(message.getCompleteResultingMessage().getValue());
            }
        }
    }
    ...
    public void adjustSshContextAfterSerialize(Message message) {
    }

    public void prepareAfterParse(Message message) {
        ProtocolMessagePreparator prep = getPreparator(message);
        prep.prepareAfterParse(sshContext.isReversePrepareAfterParse());
    }
