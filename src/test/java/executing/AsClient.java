...
public class AsClient {
    ...
    public void main() throws Exception{
        ...
        transport.sendData(new ClientInitMessageSerializer(clientInit).serialize());
        byte[] response = transport.fetchData();
       
        ClientInitMessage serverInit = new ClientInitMessageParser(0, response).parse();
        new ClientInitMessageHandler(context, serverInit).handle();
        ...
        context.appendToExchangeHashInput(Converter.concatenate(new byte[] {MessageIDConstants.SSH_MSG_KEXINIT},
                new KeyExchangeInitMessageSerializer(clientKeyInit).serializeMessageSpecificPayload()));
        
        transport.sendData(new KeyExchangeInitMessageSerializer(clientKeyInit).serialize());
        response = transport.fetchData();
        
        KeyExchangeInitMessage serverKeyInit = new KeyExchangeInitMessageParser(0, response).parse();
        new KeyExchangeInitMessageHandler(context, serverKeyInit).handle();
        ...
        transport.sendData(new ECDHKeyExchangeInitMessageSerializer(ecdhInit).serialize());
        response = transport.fetchData();
        
        ECDHKeyExchangeReplyMessage ecdhReply = new ECDHKeyExchangeReplyMessageParser(0, response).parse();
        new ECDHKeyExchangeReplyMessageHandler(context, ecdhReply).handle();
